...
class PA_LIB_PID:
    ...
    def calculate(self, sp, val, val_dot, dt):
        ...
        if pid_mod == PA_LIB_PID.PID_MODE_DERIVATIV_CALC:
            d = (error - self.error_previous) / max(dt, self.dt_min)
            self.error_previous = error
        elif pid_mod == PA_LIB_PID.PID_MODE_DERIVATIV_CALC_NO_SP:
            d = (-val - self.error_previous) / max(dt, self.dt_min)
            self.error_previous = -val
        elif pid_mod == PA_LIB_PID.PID_MODE_DERIVATIV_SET:
            d = -val_dot
        else:
            d = 0.0
        ...
        if ki > PA_LIB_PID.SIGMA:
            # Calculate the error integral and check for saturation
            i = self.integral + (error * dt)

            # check for saturation
            if math.isfinite(i):
                if (output_limit < PA_LIB_PID.SIGMA or (math.fabs(output + (i * ki)) <= output_limit)) \
        ...
        if math.isfinite(output):
            if output_limit > PA_LIB_PID.SIGMA:
